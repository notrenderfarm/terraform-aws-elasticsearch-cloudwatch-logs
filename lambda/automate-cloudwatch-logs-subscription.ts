...
export const handler: Handler = async () => {
  console.log('[terraform-aws-elasticsearch-cloudwatch-logs] Automate subscription of CloudWatch logs')
      ...
      return Promise.all(
        logs.map(async log => {
          if (!await cloudwatch.isSubscribed(log)) {
            console.log('[terraform-aws-elasticsearch-cloudwatch-logs] Subscribing', log)
